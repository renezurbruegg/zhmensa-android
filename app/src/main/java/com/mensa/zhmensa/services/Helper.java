...
public class Helper {
    ...
    static String convertMensaToJsonString(Mensa mensa) throws JsonParseException{
        if(gson == null)
    ...
    static Mensa getMensaFromJsonString(String jsonString) throws JsonParseException {
        if(gson == null)
            gson = new GsonBuilder().registerTypeAdapter(IMenu.class, new InterfaceAdapter<IMenu>())
                    .registerTypeAdapter(MensaCategory.class, new InterfaceAdapter<MensaCategory>())
                    .create();
    ...
    static int getCurrentDay() {
        ...
        if(currentDay == null) {
            currentDay = new DateTime(System.currentTimeMillis()).getDayOfWeek();

            if (currentDay > DateTimeConstants.FRIDAY)
                currentDay = 0;

            currentDay = currentDay - DateTimeConstants.MONDAY;
        }
    ...
    public static String getLabelForMealType(Mensa.MenuCategory category, Context context) {
        switch (category){
            ...
            case ALL_DAY: return context.getString(R.string.all_day);
        }
    ...
    @SuppressWarnings("HardCodedStringLiteral")
    @NonNull
    static String getLanguageCode(Context ctx) {
        if(ctx!= null)
    ...
    public static void clearDayCache() {
        currentDay = null;
    }

    public static String padToLength(String inputString, int length) {
    ...
    public static String getFullNameForDay(Mensa.Weekday day, Context ctx) {
        if(ctx != null) {
            switch (day) {
                case MONDAY:
                    return ctx.getString(R.string.monday_full);
                case TUESDAY:
                    return ctx.getString(R.string.tuesday_full);
                case WEDNESDAY:
                    return ctx.getString(R.string.wednesday_full);
                case THURSDAY:
                    return ctx.getString(R.string.thursday_full);
                case FRIDAY:
                    return ctx.getString(R.string.friday_full);
            }
        } else {
            switch (day) {
                case MONDAY:
