...
public class PollManager {
    ...
    public PollManager (Context ctx){
        ...
        /* ctx.getSharedPreferences(POLL_MANAGER_SHARED_PREF_KEY, Context.MODE_PRIVATE)
                .edit()
                .putStringSet(POLL_LIST_id_KEY, new HashSet<String>())
                .apply();
        /*Gson gson = new Gson();

        //Log.d("MensaManager.storeMensa", "Storing:" + mensaJson);
        Set<String> pollListSet = new HashSet<String>(ctx.getSharedPreferences(POLL_MANAGER_SHARED_PREF_KEY, Context.MODE_PRIVATE)
                .getStringSet(POLL_LIST_KEY, new HashSet<String>()));

        for( String pollJson : pollListSet){
            activePolls.add(gson.fromJson(pollJson, Poll.class));
        } */

    }


    private void loadPollsForId(List<String> ids) {
    ...
    private void addNewPoll(final Poll newPoll, final Context ctx, Function1<String, Void> callback) {
        ...
        PollApi.addNewPollToApi(newPoll, new Function1<JsonElement, Void>() {
            ...
            public Void invoke(JsonElement o) {
                if(o == null) {
                    ...
                    Toast.makeText(ctx, "Could not create poll at API", Toast.LENGTH_SHORT).show();
                    return null;
                }
    ...
    public void clear(Context ctx) {
        ctx.getSharedPreferences(POLL_MANAGER_SHARED_PREF_KEY, Context.MODE_PRIVATE)
                .edit()
                .putStringSet(POLL_LIST_KEY, Collections.<String>emptySet())
                .apply();
        activePolls.clear();
    ...
    public void addMensaToPoll(Mensa selectedMensa, Mensa.Weekday weekday, Mensa.MenuCategory mealType, Context ctx, final Function1<String ,Void> callbackFunction) {
    ...
    public void showAddMenusToPollDialog(final List<IMenu> menus, final String mensaId, final Context ctx, final Function1<String ,Void> callbackFunction) {
     ...
     */
    @SuppressLint("CheckResult")
    public void _showAddMenusToPollDialog(final List<IMenu> menus, final String mensaId, final Context ctx, final Function1<String ,Void> callbackFunction) {
        final List<String> list = new ArrayList<>();
        final List<Poll> polls = new ArrayList<>();

        list.add("Neue Umfrage erstellen");
        ...
        if(list.isEmpty()) {
            showCreateNewPollDialog(menus, mensaId, ctx, mealType, currentDay);
            return;
        }

        final MaterialDialog dialog = new MaterialDialog(ctx, new BottomSheet());
        dialog.title(null, "Zur Umfrage hinzufügen");
                ...
                PollApi.updatePoll(poll, new Function1<PollApi.PollApiResponse, Void>() {
                    ...
                    public Void invoke(PollApi.PollApiResponse pollApiResponse) {
                        if(pollApiResponse.error) {
                            ...
                            callbackFunction.invoke(poll.id);
                            storePolls(ctx);
                        }
                        return null;
                    }
        ...
        dialog.positiveButton(null, "Auswählen", null);

        dialog.negativeButton(null, "Abbrechen", null);
        dialog.show();
    ...
    public void showCreateNewPollDialog(final List<IMenu> menus, final String mensaId, final Context ctx, Mensa.MenuCategory mealType, Mensa.Weekday day) {
        MaterialDialog dialog = new MaterialDialog(ctx, new BottomSheet());
        ...
        et.setText("Umfrage vom " + Helper.getDayForPattern(day.day, "dd-MM"));
...
//TODO
                Poll poll = new Poll(et.getText().toString(), null, mealType, day);
