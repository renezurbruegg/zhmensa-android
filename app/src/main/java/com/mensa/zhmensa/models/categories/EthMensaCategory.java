...
public class EthMensaCategory extends MensaCategory {
    ...
    private static List<IMenu> getMenusFromJsonArray(String mensa, Mensa.MenuCategory mealType, JSONArray array) throws JSONException {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            if(!staffPrice.equals("0.00") && !staffPrice.equals("null"))
                pricesStr.append(pricesStr.length() == 0 ? "" : " / ").append(staffPrice);

            if(!externPrice.equals("0.00") && !externPrice.equals("null"))
                pricesStr.append(pricesStr.length() == 0 ? "" : " / ").append(externPrice);
    ...
    private static List<Mensa> convertJsonResponseToList(JSONArray array, Mensa.Weekday day, Mensa.MenuCategory menuCategory) {
        ...
        for (int i = 0; i < array.length(); i++) {
            try {
                ...
                JSONArray mealTimes = obj.getJSONObject("hours").getJSONArray("mealtime");

                for( int j = 0; j < mealTimes.length(); j++) {
                    JSONObject mealTimeObject = mealTimes.getJSONObject(j);
                    String timeString = mealTimeObject.getString("from") + " - " + mealTimeObject.getString("to");
                    mensa.addMealTypeTimeMapping(Mensa.MenuCategory.from(mealTimeObject.getString("type")), timeString);
                    Log.d("j", mealTimeObject.getString("type") + " -- " + timeString);
                }
    ...
    public List<MensaListObservable> loadMensasFromAPI(String languageCode) {
        ...
        for (Mensa.Weekday day: Mensa.Weekday.values()) {
            for (Mensa.MenuCategory cat: Arrays.asList(Mensa.MenuCategory.LUNCH, Mensa.MenuCategory.DINNER)) {
